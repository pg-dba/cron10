...
echo ${MSG} | mutt -e 'set content_type = text/html' -e 'set from="${MAILLOGIN}"' -e 'set realname="${MAILFROM}"' \
    ...
    -e 'set ssl_verify_dates=no' -e 'set ssl_verify_host=no' -s "PostgreSQL Locks on ${HOST}:${PORT}" "${MAILTO}"
...
#    -e 'set realname="${FROM}"' -e 'set smtp_url="${MAILSMTPURL}"' -s 'Locks "${MAILFROM}"' '"${MAILTO}"'"
